...
namespace Flame {
  ...
  BvhOld::BvhOld(std::vector<IHitableWithBounds*>& hitables): m_hitables(hitables) {
  ...
  bool BvhOld::Hit(const Ray& r, HitRecordOld& record, float tMin, float tMax) const {
    return HitNode(0, r, record, tMin, tMax);
  }

  void BvhOld::InitBounds() {
  ...
  AabbOld BvhOld::CalculateBound(const std::vector<uint32_t>& boundsId) const {
  ...
  glm::vec3 BvhOld::CalculateSplitPoint(const std::vector<uint32_t>& boundsId) const {
  ...
  uint32_t BvhOld::InitNode(std::vector<uint32_t>&& boundsId, bool shouldSubdivide) {
    uint32_t currentIndex = static_cast<uint32_t>(m_nodes.size());
    BvhNodeOld& node = m_nodes.emplace_back();
  ...
  bool BvhOld::HitNode(uint32_t nodeId, const Ray& r, HitRecordOld& record, float tMin, float tMax) const {
    // BVH hit algorithm:
    ...
    const BvhNodeOld& node = m_nodes[nodeId];

    // If bound wasn't hit - no hit
    {
      HitRecordOld record0;
    ...
    if (!node.m_boundsId.empty()) {
      HitRecordOld record0 { .time = tMax };
      bool anyHit = false;
